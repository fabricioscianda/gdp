...
import gdp.dao.carrera.ICarreraDAO;
...
public class CarreraService {
	...
	@RequestMapping(value = "/carrera/editar", method = RequestMethod.POST, produces = { "application/json; charset=UTF-8" })
	public String editar(@RequestBody String data) {
		...
		try {
			emfh.beginTransaction(em);
			VOCarrera carrera = carreraDAO.modificar(voCarrera, em);
			emfh.commitTransaction(em);
			voResponse.setData(gson.toJson(carrera));
			voResponse.setOk(true);
		} catch (Exception e) {
			emfh.rollbackTransaction(em);
			voResponse.setErrorMessage("No pudo guardarse la carrera.");
			voResponse.setOk(false);
		}
		return gson.toJson(voResponse);
	}
